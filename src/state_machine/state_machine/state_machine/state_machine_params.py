    ...
    def parameters_callback(self, parameters: List[Parameter]) -> SetParametersResult:
        for param in parameters:
            match param.name:
                ...
                case "initial_state":
                    try:
                        new_state = StateType(param.value)
                    except ValueError:
                        self.node.get_logger().error(
                            f"Invalid initial_state '{param.value}'. "
                            f"Allowed states: {[s.value for s in StateType]}"
                        )
                        return SetParametersResult(successful=False)

                    self.node.state = new_state
                    self.initial_state = param.value
