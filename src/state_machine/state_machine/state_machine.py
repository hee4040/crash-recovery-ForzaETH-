...
class StateMachine(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"[STATE_MACHINE] mode={self.params.mode}, initial_state={self.params.initial_state}"
        )
        ...
        # ----프로젝트용 코드(Collision & PF stable 구독) -----
        self.collision_detected = False
        self.pf_stable = True  # 기본은 안정 상태라고 두자
        self.backward_done = False

        self.create_subscription(
            Bool,
            '/collision_detected',      # 실제 사용 중인 토픽 이름으로 맞추기
            self.collision_cb,
            10,
        )
        self.create_subscription(
            Bool,
            '/pf/stable',  # 실제 PF stable 토픽 이름
            self.pf_stable_cb,
            10,
        )
        self.create_subscription(Bool, "/tln/backward_done", self.backward_done_cb, 10)
        #-------프로젝트용 코드-----------------------------------

        # INITIALIZATIONS        
        self.waypoints_dist = 0.1
        
        # ---------프로젝트용 코드(initial_state 파라미터를 StateType으로 변환)-----
        try:
            self.state = StateType(self.params.initial_state)
        except ValueError:
            self.get_logger().warn(
                f"Unknown initial_state '{self.params.initial_state}', fallback to NORMAL"
            )
        ...
        # 마지막 상태 기록용 (로그에 state 변화만 찍기 위해)
        self._last_state_for_log = None
    ...
    def pf_stable_cb(self, msg: Bool):
        self.pf_stable = bool(msg.data)
        if msg.data:
            self.pf_stable = True

    def backward_done_cb(self, msg: Bool):
        self.backward_done = bool(msg.data)
    #------------프로젝트용 코드--------------
    ...
    def _publish_ctrl_enables_if_changed(self):
        ...
        timetrials 모드에서는 NORMAL/RECOVERY 상태만으로 제어자를 스위칭한다.
        """
        # === [1] collision 기반 NORMAL/RECOVERY FSM (timetrials 모드에서만) ===
        if self.params.mode == 'timetrials':
            if self.state == StateType.RECOVERY:
                # 복구 모드: TLN만 동작
                self._set_ctrl_enables(pp_enable=False, tln_enable=True)
            else:
                # 나머지는 전부 NORMAL로 보고 PPF만 동작
                self._set_ctrl_enables(pp_enable=True, tln_enable=False)
            return

        # === [2] 그 외 모드(head_to_head 등)는 기존 섹터 로직 사용 ===
...
#-----------프로젝트용 코드--------------
    ...
    def visualize_state(self, state: StateType):
        ...
        if state == StateType.NORMAL:
            mrk.color.r = 0.0
            mrk.color.g = 1.0
            mrk.color.b = 0.0     # green

        elif state == StateType.RECOVERY:
            mrk.color.r = 1.0
            mrk.color.g = 0.5
            mrk.color.b = 0.0     # orange

        elif state == StateType.GB_TRACK:
            mrk.color.g = 1.0
    ...
    def main_loop_callback(self):
        ...
        if self.state != prev_state:
            self.get_logger().info(
                f"[FSM] State changed: {prev_state.value} → {self.state.value} | "
                f"collision={getattr(self, 'collision_detected', None)} | "
                f"pf_stable={getattr(self, 'pf_stable', None)} | "
                f"mode={self.params.mode}"
            )
